...
def prepare_dataset(config: dict) -> torch.utils.data.Dataset:
    ...
    if dataset_type == "lerobot":
        from dataset.lerobot_dataset_pretrain_mp_3in import LeRobotDataset 
        import yaml
        with open(config.get("dataset_config_path"), 'r') as f:
            dataset_config = yaml.safe_load(f)
        print("DATASET CONFIG LOADED:", dataset_config)
...
def log_training_step(step, loss, total_norm, clipped_norm, scheduler, dataloader, accelerator):
   ...
   #     })//
def save_checkpoint(save_dir, step, model_engine, loss, accelerator,
                    config=None, norm_stats=None,
    ...
    # save optimizer/scheduler only if provided
...
#             print(f"DeepSpeed save failed: {e}")
...
#         print(f"Saved PyTorch checkpoint at {checkpoint_dir}")
    # Try DeepSpeed-style save only if available
def load_checkpoint_with_deepspeed(model_engine, load_dir, accelerator, tag="step_5000", **kwargs):

    ckpt_path = os.path.join(load_dir, tag, "checkpoint.pt")
    ...
    model_to_load = model_engine.module if hasattr(model_engine, "module") else model_engine
...
def train(config):
    ...
    # ══════════════════════════════════════════════════════════════
    # SHAPE VERIFICATION — runs once at startup, remove after confirmed
    # ══════════════════════════════════════════════════════════════
    if accelerator.is_main_process:
        logging.info("\n" + "═"*60)
        logging.info("TEMPORAL DATASET SHAPE VERIFICATION (first 3 samples)")
        logging.info("═"*60)
        ...
        for sample_idx in range(min(3, len(dataset))):
            ...
            images     = sample["images"]      # expect (3, max_views, 3, H, W)
            state      = sample["state"]       # expect (3, max_state_dim)
            action     = sample["action"]      # expect (horizon, per_action_dim) padded
            image_mask = sample["image_mask"]  # expect (max_views,)
            state_mask = sample["state_mask"]  # expect (max_state_dim,)
            ...
            logging.info(f"  image_mask  shape : {image_mask.shape}")
            logging.info(f"  state_mask  shape : {state_mask.shape}")
            logging.info(f"  prompt            : '{prompt}'")
            logging.info(f"  N_OBS_STEPS (dim0 images) : {images.shape[0]}  ← must be 3")
            logging.info(f"  N_OBS_STEPS (dim0 state)  : {state.shape[0]}   ← must be 3")
            logging.info(f"  prompt same for all steps : YES (single string per sample)")

            # Print state values for all 3 timesteps so you can see they differ
            state_f32 = state.float()
            for t in range(state.shape[0]):
                valid_dims = state_mask.sum().item()
                logging.info(
                    f"  state[t={t}] first {int(valid_dims)} valid dims: "
                    f"{state_f32[t, :int(valid_dims)].tolist()}"
                )

            # ── visual grid: 3 timesteps × num_valid_views ──────────────────
            n_steps     = images.shape[0]                        # 3
            n_views     = int(image_mask.sum().item())           # real cameras
            fig, axes   = plt.subplots(
                n_steps, max(n_views, 1),
                figsize=(4 * max(n_views, 1), 4 * n_steps),
                squeeze=False
            )
            fig.suptitle(
                f"Sample {sample_idx} | prompt: '{prompt[:60]}'\n"
                f"images shape: {tuple(images.shape)}  "
                f"state shape: {tuple(state.shape)}",
                fontsize=10
            )
            for t in range(n_steps):
                for v in range(n_views):
                    # images is bfloat16, convert to float32 for display
                    img_tensor = images[t, v].float()            # (3, H, W)
                    # un-normalise from ImageNet stats for display
                    mean = torch.tensor([0.485, 0.456, 0.406]).view(3,1,1)
                    std  = torch.tensor([0.229, 0.224, 0.225]).view(3,1,1)
                    img_display = img_tensor * std + mean
                    img_display = img_display.clamp(0, 1).permute(1, 2, 0).numpy()
                    axes[t][v].imshow(img_display)
                    axes[t][v].set_title(
                        f"t-{n_steps-1-t} | view {v}", fontsize=8
                    )
                    axes[t][v].axis("off")
                # blank out unused view columns
                for v in range(n_views, axes.shape[1]):
                    axes[t][v].axis("off")

            save_path = os.path.join(save_dir, f"temporal_sample_{sample_idx}.png")
            plt.tight_layout()
            plt.savefig(save_path, dpi=120)
            plt.close(fig)
            logging.info(f"  Visual saved → {save_path}")

        logging.info("\n" + "═"*60)
        logging.info("EXPECTED SHAPES SUMMARY")
        logging.info("  images   : (N_OBS_STEPS=3, max_views, 3, 448, 448)")
        logging.info("  state    : (N_OBS_STEPS=3, max_state_dim)")
        logging.info("  action   : (horizon, per_action_dim_padded)  — anchor t only")
        logging.info("  prompt   : str  — same task description for all 3 steps")
        logging.info("═"*60 + "\n")
    # ══════════════════════════════════════════════════════════════
    ...
    while step < max_steps:
        for batch in tqdm(dataloader, desc="Training", disable=not accelerator.is_main_process):
            ...
            # states shape: (B, N_OBS_STEPS, max_state_dim)
            # Flatten timesteps into feature dim for the state projector:
            # (B, N_OBS_STEPS, max_state_dim) → (B, N_OBS_STEPS * max_state_dim)
            B = states.shape[0]
            states_flat = states.view(B, -1)   # (B, N_OBS_STEPS * max_state_dim)
            ...
            for prompt, images, image_mask in zip(prompts, images_batch, image_masks):
                # images shape: (N_OBS_STEPS, max_views, 3, H, W)
                # We encode each timestep separately through the VLM, then
                # concatenate along the token dimension so the action head
                # sees [tokens_t-2 | tokens_t-1 | tokens_t] as context.
                n_steps = images.shape[0]   # 3
                step_tokens = []
                for t in range(n_steps):
                    images_t = images[t]    # (max_views, 3, H, W) — single timestep
                    fused_t = model.get_vl_embeddings(
                        images=images_t,
                        image_mask=image_mask,
                        prompt=prompt,          # same prompt for all 3 steps
                        return_cls_only=False
                    )
                    if step == 0 and t == 0 and accelerator.is_main_process:
                        print(f"images_t shape (one timestep into VLM) : {images_t.shape}")
                        print(f"fused_t shape (tokens from one timestep): {fused_t.shape}")                                               # → (1, n_tokens, embed_dim)
                    step_tokens.append(fused_t)

                # cat along token dim → (1, n_tokens * N_OBS_STEPS, embed_dim)
                fused_sample = torch.cat(step_tokens, dim=1)
                if step == 0 and accelerator.is_main_process:
                    print(f"fused_sample shape (3 timesteps concat)  : {fused_sample.shape}")
                fused_tokens_list.append(fused_sample.to(dtype=torch.bfloat16))

            fused_tokens = torch.cat(fused_tokens_list, dim=0)
            if step == 0 and accelerator.is_main_process:
                print(f"fused_tokens final shape (full batch)     : {fused_tokens.shape}")
# ── END ADD ─────────────────────────────────────────────────
            # shape: (B, n_tokens * N_OBS_STEPS, embed_dim)

            with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):

                pred_velocity, noise = model(fused_tokens, state=states_flat, actions_gt=actions_gt, action_mask=action_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action_head", type=str, default="flowmatching", choices=["flowmatching", "flowmatching_3in"])
