...
def _process_parquet_file_worker(args):
    parquet_path, arm_name, dataset_name, dataset_config, dataset_path, task_mapping, action_horizon, max_samples_per_file, cache_dir, n_obs_steps = args
...
class LeRobotDataset(Dataset):
    ...
    def __init__(
        ...
        if cache_dir is None:
            self.cache_dir = Path("/home/qxc857/Evo-1/Evo_1/training_data_cache/")
        else:
    ...
    N_OBS_STEPS = 3   # class constant: t-2, t-1, t

    def __getitem__(self, idx):

        # ── Step 1: derive temporal pkl paths from the anchor filename ───────────
        #
        # Cache layout:  cache_dir/<arm>/<dataset>/chunk-000/<episode_stem>/<start>_<end>.pkl
        # e.g.           cache/metaworld/mt50/chunk-000/episode_000042/5_55.pkl
        #
        # start_idx == frame_index of the anchor row in the parquet.
        # t-1 == (start-1)_(end-1).pkl  in the SAME episode folder.
        # t-2 == (start-2)_(end-2).pkl  in the SAME episode folder.
        # If that file doesn't exist (episode starts at frame 0, 1),
        # we pad by repeating the earliest file we already found.

        anchor_path = Path(self.data[idx])
        episode_dir = anchor_path.parent                 # .../episode_000042/

        start, end = map(int, anchor_path.stem.split("_"))   # e.g. 5, 55
        horizon = end - start                            # == action_horizon (e.g. 50)

        temporal_paths = []
        for step in range(-(self.N_OBS_STEPS - 1), 1):  # [-2, -1, 0]
            s = start + step                             # e.g. 3, 4, 5
            candidate = episode_dir / f"{s}_{s + horizon}.pkl"
            if s < 0 or not candidate.exists():
                # pad: repeat earliest valid frame found so far, else anchor
                candidate = temporal_paths[0] if temporal_paths else anchor_path
            temporal_paths.append(candidate)
        # temporal_paths = [path_t-2, path_t-1, path_t]   (oldest → newest)

        # ── Step 2: load all 3 pkl items ─────────────────────────────────────────
        items = []
        for t_path in temporal_paths:
            try:
                ...
                logging.info(f"cannot load cache file {t_path}: {str(e)}")
                return self[random.randint(0, len(self.data) - 1)]

        # anchor item (t) drives everything except images and state
        item = items[-1]
        ...
        # ── Step 3: load one video frame per timestep ─────────────────────────────
        # All 3 pkls point to the same episode_000042.mp4 but different timestamps.
        # _load_video_frame seeks to the right frame using the timestamp field.
        all_frame_lists = []
        for t_item in items:
            try:
                frames = self._load_video_frame(
                    t_item["video_paths"], t_item["timestamp"]
                )
            except Exception as e:
                logging.info(f"skipping sample, cannot decode video: {e}")
                return self[random.randint(0, len(self.data) - 1)]
            all_frame_lists.append(frames)
        # all_frame_lists[0] = frames at t-2
        # all_frame_lists[1] = frames at t-1
        # all_frame_lists[2] = frames at t

        # ── Step 4: augment / transform each timestep independently ───────────────
        all_images_per_step = []
        for frames in all_frame_lists:
            imgs = []
            for img in frames:
                if self.use_augmentation and random.random() < 0.5:
                    imgs.append(self.aug_transform(img))
                else:
                    imgs.append(self.basic_transform(img))

            num_real_views = len(imgs)
            image_mask = torch.zeros(self.max_views, dtype=torch.bool)
            image_mask[:num_real_views] = True

            while len(imgs) < self.max_views:
                dummy = (
                    torch.zeros(3, 448, 448)
                    if len(imgs) == 0
                    else torch.zeros_like(imgs[0])
                )
                imgs.append(dummy)

            all_images_per_step.append(torch.stack(imgs))  # (max_views, 3, H, W)

        # stack timesteps → (N_OBS_STEPS, max_views, 3, H, W)
        images = torch.stack(all_images_per_step, dim=0)

        # ── Step 5: normalise + stack states for all 3 timesteps ──────────────────
        try:
            norm_stats = self.arm2stats_dict[arm_key]
        except KeyError:
            raise KeyError(
                f"Normalization stats not found for arm_key={arm_key}, "
                f"dataset_key={dataset_key}"
            )

        state_min = torch.tensor(
            norm_stats["observation.state"]["min"], dtype=torch.float32
        )
        state_max = torch.tensor(
            norm_stats["observation.state"]["max"], dtype=torch.float32
        )

        states_per_step = []
        for t_item in items:
            if t_item["state"] is None:
                raise ValueError(
                    "missing observation.state, please check data integrity"
                )
            s = torch.tensor(t_item["state"], dtype=torch.float32)
            s = 2 * (s - state_min) / (state_max - state_min + 1e-8) - 1
            s = torch.clamp(s, -1.0, 1.0)
            s_padded, _ = self._pad_tensor(s, self.max_state_dim)
            states_per_step.append(s_padded)

        # (N_OBS_STEPS, max_state_dim)
        state_stacked = torch.stack(states_per_step, dim=0)

        # state_mask from anchor only (same embodiment layout across all steps)
        anchor_state = torch.tensor(item["state"], dtype=torch.float32)
        _, state_mask = self._pad_tensor(anchor_state, self.max_state_dim)

        # ── Step 6: action from anchor frame t only (future horizon) ──────────────
        if item["action"] is None:
            raise ValueError("missing action, please check data integrity")

        action = torch.from_numpy(np.stack(item["action"])).float()
        action_min = torch.tensor(
            norm_stats["action"]["min"], dtype=torch.float32, device=action.device
        )
        action_max = torch.tensor(
            norm_stats["action"]["max"], dtype=torch.float32, device=action.device
        )
        action = (
            2 * (action - action_min.unsqueeze(0))
            / (action_max.unsqueeze(0) - action_min.unsqueeze(0) + 1e-8)
            - 1
        )
        action = torch.clamp(action, -1.0, 1.0)
        action_padded, action_mask = self._pad_tensor(action, self.max_action_dim)

        # ── Step 7: prompt unchanged — same task description for entire episode ────
        prompt = item["prompt"] if item["prompt"] is not None else ""

        return {
            # CHANGED shape: (N_OBS_STEPS, max_views, 3, H, W)
            "images":        images,
            "image_mask":    image_mask,                         # (max_views,) — unchanged

            # CHANGED shape: (N_OBS_STEPS, max_state_dim)
            "state":         state_stacked.to(dtype=torch.bfloat16),
            "state_mask":    state_mask,                         # (max_state_dim,) — unchanged
